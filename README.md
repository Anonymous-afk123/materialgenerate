...
用户在设置页必须填写 Base URL、模型和 API Key。API Key 会立即在服务端使用 AES-256-GCM 加密写入 `llm_configs`；浏览器只保留配置 ID、Base URL、模型和末四位。生成或 AI 补全时服务端按当前账户解密使用。
...
按文件名顺序执行 `supabase/migrations/` 下的 SQL 文件，创建 Auth 用户关联的申请、著作权人、生成任务、事件、生成记录、源码包、索引和 RLS。历史材料/浏览器自动化相关表结构保留在迁移中，不再由当前网站使用。Storage 中创建名为 `generated-documents` 的私有 Bucket。DOCX 由现有 Python 模板生成，PDF 由当前生成链路处理。
...
- 源代码、用户手册同时生成 DOCX/PDF；源码压缩包可在申请编辑页作为生成输入。
